...
class GraphAnalyzer:
    ...
    def _load_graph(self):
        ...
        # 加载社区报告并创建community节点
        self._load_communities()
    
    def _load_communities(self):
        """从社区报告文件中加载社区并创建community节点"""
        import os
        import glob
        
        # 构建社区报告文件路径（支持多种命名模式）
        graph_dir = os.path.dirname(self.graph_path)
        graph_base = os.path.splitext(os.path.basename(self.graph_path))[0]
        
        # 尝试多种可能的社区报告文件路径
        possible_paths = [
            os.path.join(graph_dir, f"{graph_base}_community_report.json"),  # graph_doc_0000_community_report.json
            os.path.join(graph_dir, "multi_stage_deduplicated_community_report.json"),  # 旧命名
        ]
        
        # 也尝试查找目录下任意 *_community_report.json 文件
        comm_report_path = None
        for p in possible_paths:
            if os.path.exists(p):
                comm_report_path = p
                break
        
        if comm_report_path is None:
            # 兜底：查找任意 *_community_report.json
            pattern = os.path.join(graph_dir, "*_community_report.json")
            matches = glob.glob(pattern)
            if matches:
                comm_report_path = matches[0]
        
        if comm_report_path is None or not os.path.exists(comm_report_path):
            logger.info(f"社区报告文件不存在于: {graph_dir}")
            return
        
        try:
            with open(comm_report_path, 'r', encoding='utf-8') as f:
                report_data = json.load(f)
            
            communities = report_data.get('community_report', [])
            if not communities:
                logger.info("社区报告为空")
                return
            
            logger.info(f"正在加载 {len(communities)} 个社区...")
            
            # 为每个社区创建community节点
            for i, comm in enumerate(communities):
                comm_name = comm.get('name', f"Community_{i}")
                
                # 处理name可能是列表的情况
                if isinstance(comm_name, list):
                    comm_name = comm_name[0] if comm_name else f"Community_{i}"
                if not isinstance(comm_name, str):
                ...
                self.graph.add_node(
                    ...
                    properties={
                        "name": comm_name,
                        "description": comm.get('description', '') if isinstance(comm.get('description', ''), str) else str(comm.get('description', '')),
                        "summary": summary,
                        "keywords": comm.get('keywords', []) if isinstance(comm.get('keywords', []), list) else [],
                        "member_count": len(comm.get('members', [])) if isinstance(comm.get('members', []), list) else 0
                    }
                )
                ...
                for member_name in members:
                    # 查找成员节点（通过名称匹配）
                    member_id = self._find_node_by_name(member_name)
                    if member_id:
                        # 添加 member -> community 的边
                        self.graph.add_edge(
                            member_id,
                            comm_id,
                            relation="belongs_to_community",
                            properties={}
                        )
            
            logger.info(f"社区节点加载完成: 添加了 {len(communities)} 个community节点")
            
        except Exception as e:
            logger.warning(f"加载社区报告失败: {e}")
    
    def _find_node_by_name(self, name) -> Optional[str]:
        """
        根据节点名称查找节点ID
        ...
        for node_id, node_data in self.graph.nodes(data=True):
            ...
            # 直接匹配
            if node_name.lower().strip() == name_lower:
                return node_id
            
            # 检查aliases（如果有）
            aliases = extract_aliases(node_name)
            for alias in aliases:
                if isinstance(alias, str) and alias.lower().strip() == name_lower:
                    return node_id
    ...
    def convert_to_echarts_format(self, graph: Optional[nx.MultiDiGraph] = None, max_nodes: int = 1000,
        ...
        if graph.is_directed():
            for u, v, edge_data in graph.edges(data=True):
                ...
                if rel in ("member_of", "belongs_to_community"):
        ...
        if total_nodes > threshold_high and not is_subgraph:
            # 只展示community节点，且只保留成员数 >= 5 的社区
            filtered_nodes = []
            for node_id in all_nodes:
                if graph.nodes[node_id].get("label") == "community":
                    member_count = graph.nodes[node_id].get("properties", {}).get("member_count", 0)
            ...
            if filtered_nodes:
                ...
                logger.info(f"总节点数({total_nodes})大于{threshold_high}，只展示成员>=5的community节点: {len(all_nodes)} 个")
                # 构建前端通知信息
                display_notification = {
                    "type": "high_threshold_filter",
                    "title": "节点数量过多，已自动筛选显示",
                    "message": f"当前图谱共有 {total_nodes} 个节点，超过了高阈值({threshold_high})。为优化展示效果，系统已自动筛选仅显示成员数≥5的社区(community)节点，共 {len(all_nodes)} 个。如需查看完整图谱，请使用子图查询功能或调整阈值设置。",
                    "level": "info",
                    "duration": 8000,
                    "total_nodes": total_nodes,
                    "displayed_nodes": len(all_nodes),
                ...
                # 构建回退通知信息
                display_notification = {
                    "type": "fallback_to_subgraph",
                    "title": "未找到社区节点，回退到完整显示",
                    "message": f"当前图谱共有 {total_nodes} 个节点（超过高阈值{threshold_high}），但未找到社区(community)节点。系统将回退到子图模式，展示所有节点。如需优化显示，建议重新构建图谱以生成社区结构。",
                    "level": "warning",
                    "duration": 10000,
                    "total_nodes": total_nodes,
                    "threshold_high": threshold_high
            ...
            if filtered_nodes:
                ...
                # 构建前端通知信息
                display_notification = {
                    "type": "medium_threshold_filter",
                    "title": "节点数量较多，已自动筛选显示",
                    "message": f"当前图谱共有 {total_nodes} 个节点，处于中阈值范围({threshold_medium}-{threshold_high})。为优化展示效果，系统已自动筛选显示社区(community)和关键词(keyword)节点，共 {len(all_nodes)} 个。如需查看完整图谱，请使用子图查询功能或调整阈值设置。",
                    "level": "info",
                    "duration": 8000,
                    "total_nodes": total_nodes,
                    "displayed_nodes": len(all_nodes),
                ...
                display_notification = {
                    "type": "fallback_to_subgraph",
                    "title": "未找到社区/关键词节点，回退到完整显示",
                    "message": f"当前图谱共有 {total_nodes} 个节点（处于中阈值范围{threshold_medium}-{threshold_high}），但未找到社区(community)或关键词(keyword)节点。系统将回退到子图模式，展示所有节点。如需优化显示，建议重新构建图谱以生成社区和关键词结构。",
                    "level": "warning",
                    "duration": 10000,
                    "total_nodes": total_nodes,
                    "threshold_medium": threshold_medium,
        ...
        if len(all_nodes) > max_nodes and not is_subgraph:
            # 只对完整图进行采样，按度排序，保留最重要的节点
            node_degrees = {node: graph.degree(node) for node in all_nodes}
            sorted_nodes = sorted(all_nodes, key=lambda n: node_degrees[n], reverse=True)
        ...
        # 当只显示community节点且links为空时，不做特殊处理
        # 前端 renderGraph 会检测这种情冴并自动切换到 circular 布局
        ...
        if display_notification:
            result["display_notification"] = display_notification
